	...
	methods: {
		...
		selectGoods(data) { //选择数据
			data.forEach((el, index) => {
				el.goods_num = el.real_stock ? el.real_stock + 0 : 1;
				el.title = el.sku_name + ''
                //点击或回车行为选择商品后：当为第一行并且展示列表不存在选择的商品push到展示列表，或者不为第一行并且展示列表中不存在时选择的商品除第一条全部push到展示列表
				if ((this.inputIndex === -1 || index) && !this.goodsIdArr.includes(el.sku_id)) {
					this.goodsIdArr.push(el.sku_id);
					this.goodsList.push(el);
				} else if (!this.goodsIdArr.includes(el.sku_id)) {//当前不为第一行并且当前选择的商品第一条不存在展示列表，替换当前行数据
					this.goodsIdArr.splice(this.inputIndex, 1, el.sku_id)
					this.goodsList.splice(this.inputIndex, 1, el)
				} else {//只要展示列表存在直接累加
