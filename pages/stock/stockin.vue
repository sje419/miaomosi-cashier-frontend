					...
					<view class="table-body">
						<view class="table-tr">
							<view class="table-td select-goods-input" style="flex: 3;" @click.stop="goodsShow = true">
								<input type="text" @confirm="getGoodsData($event, -1)" placeholder="请输入产品名称/规格/编码" />
						...
						<block v-for="(item, index) in goodsList" :key="index">
							<view class="table-tr" v-if="goodsIdArr.includes(item.sku_id)">
								<view class="table-td goods-name" style="flex: 3;">
									<input type="text" v-model="item.title" @confirm="getGoodsData($event, index)" placeholder="请输入产品名称/规格/编码" />
									<text class="iconfont icontuodong" @click="getGoodsData({ detail: null }, index)"></text>
								</view>
