		...
		calculation() {
			...
			calculate(data).then(res => {
				if (res.code == 0) {
					...
					let billingGoodsDataKeys = Object.keys(this.billingGoodsData);
					//console.log(billingGoodsDataKeys, 'billingGoodsDataKeys');
					calculateOrderData.goods_list.forEach((item, index) => {
						item.editKey = billingGoodsDataKeys[index];
					});
